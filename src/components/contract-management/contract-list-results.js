...
export const ContractListResults = ({ contracts,handleOpenContractDetails, ...rest }) => {
  ...
  const handleSelectAll = (event) => {
    ...
    if (event.target.checked) {
      newSelectedContractIds = contracts.map((customer) => customer.id);
    } else {
      ...
      <PerfectScrollbar>
        <Box sx={{ minWidth: 1050 }}>
          <Table>
            ...
            <TableBody>
              {contracts.slice(0, limit).map((player, key) => (
                <TableRow
